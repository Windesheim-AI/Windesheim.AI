...
const ASSISTANT_ID = 'asst_XHthfsMOHcTOehUweObRkqxP'; // Replace with your actual Assistant ID

export async function fetchChatResponse(messages: { role: "user"; content: string }[]) {
  try {
    ...
    if (!thread.id) {
      throw new Error(`Failed to create thread: ${JSON.stringify(thread)}`);
    }

    // Step 2: Add user message to thread
    const lastUserMessage = messages[messages.length - 1];

    const addMessageRes = await fetch(`https://api.openai.com/v1/threads/${thread.id}/messages`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${OPENAI_API_KEY}`,
        "Content-Type": "application/json",
    ...
    const textContent = assistantMessage?.content?.find((c: any) => c.type === "text")?.text?.value;
